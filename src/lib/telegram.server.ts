...
export async function sendTelegramMessage(chatId: number | string, text: string) {
  ...
  if (!lovableKey || !tgKey) {
    // Skip sending instead of crashing when the connector is not wired up yet.
    console.warn("Telegram not configured (missing LOVABLE_API_KEY or TELEGRAM_API_KEY); skipping message.");
    return null;
  }
